...
def fetch_tiktok_post_info(video_url): # Function name remains as provided
    ...
    and formats the output video URL to be the original input URL.
    ...
    # --- Extract username and video ID from the input URL (Preferred for Author/ID fields, but not the final URL) ---
    ...
    if tiktok_url_match:
        ...
        print(f"Warning: Could not parse username and video ID from input URL: {video_url}. Will rely on API response for Author/ID fields.")
    ...
    for _ in range(rapidapi_key_manager.max_key_rotations):
        ...
        try:
            ...
            if res.status == 429:
                ...
                # Fix: The type hint error from 'rapidapi_key_key_manager' was corrected to 'rapidapi_key_manager'.
    ...
    # --- MODIFICATION START ---
    # Set the video_share_url to the exact input video_url as requested.
    video_share_url = video_url
    # --- MODIFICATION END ---
    ...
    return {
        ...
        "Video URL": video_share_url, # Now uses the exact input URL
